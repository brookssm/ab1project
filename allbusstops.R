library(dplyr)
# Almost all the other below code became pointless when I found a simple csv
all_stops <- read.csv("./data/puget sound gtfs/stops.txt", stringsAsFactors = F, fileEncoding = "latin1") %>% 
  setNames(gsub("stop_", "", colnames(.))) %>% 
  select(id, name, lat, lon)

agencies <- read.csv("./data/puget sound gtfs/agency.txt", stringsAsFactors = F, fileEncoding = "latin1")
